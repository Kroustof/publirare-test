...
export default function Register() {
  ...
  const { data: userData } = useMoralisQuery(
    "_User",
  ...
  const handleRegister = async (e) => {
    ...
    const registerInfo = {
      ...
      isEditor: isEditor,
      editor: editorNameRef.current.value || "selfEdition",
      isPremium: false,
  ...
  useEffect(() => {
    if (userData.length !== 0) {
      const isCreator = userData[0].get("isCreator")
      if (isCreator) {
        router.push('/')
      }
    }
  ...
  if (isAuthenticated && !isAuthUndefined && userData && userData.length !== 0) {
    return (
