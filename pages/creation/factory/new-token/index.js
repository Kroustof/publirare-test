...
import ContractChoices from "../../../../components/Creation/Factory/ContractChoices"
...
export default function NewToken() {
  ...
  const validation = () => {
    if (!selectedExtension || !selectedSize || !selectedContract) return
    const format = selectedFormat
    const extension = selectedExtension
    const size = selectedSize
    router.push(`/creation/factory/new-token/create-token?contract=${selectedContract}&format=${format}&extension=${extension}&size=${size}`)
  }
  ...
  if (isAuthenticated && isCreator && creatorInfos.isPremium && !isLoading) {
    return (
      <div className="pb-32 flex flex-col">
        <h2 className="self-center text-center text-xl text-gray-700 font-extrabold uppercase">Select a contract collection</h2>
        <ContractChoices 
        ...
        <div className="flex justify-center">
          <button
            type="button"
            onClick={validation}
            disabled={!selectedFormat || selectedExtension === null || selectedSize === null || selectedContract === ""}
...
NewToken.getLayout = function getLayout(page) {
  return (
    <LayoutSmall>
      {page}
    </LayoutSmall>
